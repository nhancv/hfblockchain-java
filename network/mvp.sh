...
function printHelp() {
  ...
  echo "	mvp.sh up -c mvpchannel -s couchdb"
  echo "        mvp.sh up -c mvpchannel -s couchdb -i 1.2.x"
...
# Using COMPOSE_FILE_ROOT, replace constants with private key file names
...
function generateNetworkConfig() {
  ...
  # Copy the template to the file that will be modified to add the private key
  cp docker-compose/network-config-root.json docker-compose/network-config.json
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose/network-config.json
  cd crypto-config/peerOrganizations/org2.beesightsoft.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose/network-config.json
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/ADMIN1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose/network-config.json
  cd crypto-config/peerOrganizations/org2.beesightsoft.com/users/Admin@org2.beesightsoft.com/msp/keystore/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/ADMIN2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose/network-config.json

  # If MacOSX, remove the temporary backup of the docker-compose file
  if [ "$ARCH" == "Darwin" ]; then
    rm docker-compose/network-config.jsont
  fi
}
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile TwoOrgsOrdererGenesis -outputBlock ./config/genesis.block -channelID genesis
  res=$?
  set +x
  if [ $res -ne 0 ]; then
